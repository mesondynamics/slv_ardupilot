...
//links the yutong uavcan message to this backend
bool AP_YuTong_DroneCAN::subscribe_msgs(AP_DroneCAN* ap_dronecan)
{
...
AP_YuTong_DroneCAN* AP_YuTong_DroneCAN::get_dronecan_backend(AP_DroneCAN* ap_dronecan, uint8_t node_id, uint8_t address, bool create_new)
{
    if (ap_dronecan == nullptr) {
        return nullptr;
    }
    AP_YuTong_DroneCAN* driver = nullptr;
    YuTong &frontend = *AP::yutong();
    //Scan through the Yutong params to find UAVCAN RFND with matching address.
    for (uint8_t i = 0; i < YUTONG_MAX_INSTANCES; i++) {
        if ((YuTong::Type)frontend.params[i].type.get() == YuTong::Type::UAVCAN &&
            frontend.params[i].address == address) {
            driver = (AP_YuTong_DroneCAN*)frontend.drivers[i];
        }
        //Double check if the driver was initialised as UAVCAN Type
        if (driver != nullptr && (driver->_backend_type == YuTong::Type::UAVCAN)) {
    ...
    if (create_new) {
        for (uint8_t i = 0; i < YUTONG_MAX_INSTANCES; i++) {
            if ((YuTong::Type)frontend.params[i].type.get() == YuTong::Type::UAVCAN &&
                ...
                frontend.drivers[i] = NEW_NOTHROW AP_YuTong_DroneCAN(frontend.state[i], frontend.params[i]);
                driver = (AP_YuTong_DroneCAN*)frontend.drivers[i];
                if (driver == nullptr) {
                    break;
                }
                GCS_SEND_TEXT(MAV_SEVERITY_INFO, "YuTong[%u]: added DroneCAN node %u addr %u",
...
void AP_YuTong_DroneCAN::update()
{
    WITH_SEMAPHORE(_sem);
    if ((AP_HAL::millis() - _last_reading_ms) > 500) {
        //if data is older than 500ms, report NoData
        set_status(YuTong::Status::NoData);
    } else if (_status == YuTong::Status::Good && new_data) {
        //copy over states
        state.distance_m = _distance_cm * 0.01f;
        state.last_reading_ms = _last_reading_ms;
        update_status();
        new_data = false;
    } else if (_status != YuTong::Status::Good) {
        //handle additional states received by measurement handler
        set_status(_status);
    }
}

//YuTong message handler
void AP_YuTong_DroneCAN::handle_measurement(AP_DroneCAN *ap_dronecan, const CanardRxTransfer& transfer, const uavcan_equipment_range_sensor_Measurement &msg)
{
    //fetch the matching uavcan driver, node id and sensor id backend instance
    AP_YuTong_DroneCAN* driver = get_dronecan_backend(ap_dronecan, transfer.source_node_id, msg.sensor_id, true);
    if (driver == nullptr) {
        return;
    }
    ...
    switch (msg.reading_type) {
        case UAVCAN_EQUIPMENT_RANGE_SENSOR_MEASUREMENT_READING_TYPE_VALID_RANGE:
        {
            ...
            driver->_status = YuTong::Status::Good;
        ...
        {
            driver->_last_reading_ms = AP_HAL::millis();
            driver->_status = YuTong::Status::OutOfRangeLow;
            break;
        }
        case UAVCAN_EQUIPMENT_RANGE_SENSOR_MEASUREMENT_READING_TYPE_TOO_FAR:
        {
            driver->_last_reading_ms = AP_HAL::millis();
            driver->_status = YuTong::Status::OutOfRangeHigh;
            break;
        }
        default:
        {
    ...
    //copy over the sensor type of Yutong 
