...
#ifndef YUTONG_MAX_INSTANCES 
  #if AP_YUTONG_ENABLED
  #define YUTONG_MAX_INSTANCES 10
  #else
  #define YUTONG_MAX_INSTANCES 1
  #endif
#endif

#define YUTONG_GROUND_CLEARANCE_CM_DEFAULT 10
#define YUTONG_PREARM_ALT_MAX_CM           200
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
#define YUTONG_PREARM_REQUIRED_CHANGE_CM   0
#else
#define YUTONG_PREARM_REQUIRED_CHANGE_CM   50
#endif

class AP_YuTong_Backend;

class YuTong
{
    friend class AP_YuTong_Backend;
    //UAVCAN drivers are initialised in the Backend, hence list of drivers is needed there.
    friend class AP_YuTong_DroneCAN;
public:
    YuTong();

    /* Do not allow copies */
    CLASS_NO_COPY(YuTong);
    ...
    static const struct AP_Param::GroupInfo *backend_var_info[YUTONG_MAX_INSTANCES];
    ...
    /*
      ...
      sets up yutongs with a gap in the types then this is the
    ...
    */
    ...
    // detect and initialise any available yutongs
    void init(enum Rotation orientation_default);

    // update state of all yutongs. Should be called at around
    // 10Hz from main loop
    void update(void);
    ...
    void handle_msp(const MSP::msp_yutong_data_message_t &pkt);
#endif
    // return true if we have a range finder with the specified orientation
    bool has_orientation(enum Rotation orientation) const;

    // find first range finder instance with the specified orientation
    AP_YuTong_Backend *find_instance(enum Rotation orientation) const;

    AP_YuTong_Backend *get_backend(uint8_t id) const;

    // get yutong type for an ID
    Type get_type(uint8_t id) const {
        return id >= YUTONG_MAX_INSTANCES? Type::NONE : Type(params[id].type.get());
    }

    // get yutong address (for AP_Periph CAN)
    uint8_t get_address(uint8_t id) const {
        return id >= YUTONG_MAX_INSTANCES? 0 : uint8_t(params[id].address.get());
    }
    ...
    YuTong::Status status_orient(enum Rotation orientation) const;
    ...
    static YuTong *get_singleton(void) { return _singleton; }

protected:
    AP_YuTong_Params params[YUTONG_MAX_INSTANCES];

private:
    static YuTong *_singleton;

    YuTong_State state[YUTONG_MAX_INSTANCES];
    AP_YuTong_Backend *drivers[YUTONG_MAX_INSTANCES];
    ...
    bool _add_backend(AP_YuTong_Backend *driver, uint8_t instance, uint8_t serial_instance=0);
