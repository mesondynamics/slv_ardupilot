...
void setup()
{
    ...
    hal.console->printf("YuTong library test\n");

    // setup for analog pin 13
    AP_Param::set_object_value(&sonar, sonar.var_info, "_TYPE", (uint8_t)YuTong::Type::PLI2C);
    ...
    hal.console->printf("YuTong: %d devices detected\n", sonar.num_sensors());
}
...
void loop()
{
    ...
    for (uint8_t i=0; i<sonar.num_sensors(); i++) {
        AP_YuTong_Backend *sensor = sonar.get_backend(i);
        if (sensor == nullptr) {
            continue;
        }
        if (!sensor->has_data()) {
