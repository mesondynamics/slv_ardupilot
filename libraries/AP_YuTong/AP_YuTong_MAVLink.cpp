...
*/
void AP_YuTong_MAVLink::handle_msg(const mavlink_message_t &msg)
{
    ...
    if (packet.orientation == orientation()) {
        ...
        if (signal_quality == 0) {
            ...
            signal_quality = YuTong::SIGNAL_QUALITY_UNKNOWN;
        } else if (signal_quality == 1) {
            // Map 1 to 0 as that is what ardupilot uses as the worst signal quality
            signal_quality = YuTong::SIGNAL_QUALITY_MIN;
        }
    }
}

int16_t AP_YuTong_MAVLink::max_distance_cm() const
{
...
int16_t AP_YuTong_MAVLink::min_distance_cm() const
{
    if (_max_distance_cm == 0 && _min_distance_cm == 0) {
        // we assume if both of these are zero that we ignore both
...
*/
void AP_YuTong_MAVLink::update(void)
{
    //Time out on incoming data; if we don't get new
    //data in 500ms, dump it
    if (AP_HAL::millis() - state.last_reading_ms > AP_YUTONG_MAVLINK_TIMEOUT_MS) {
        set_status(YuTong::Status::NoData);
        state.distance_m = 0.0f;
        state.signal_quality_pct = YuTong::SIGNAL_QUALITY_UNKNOWN;
    } else {
