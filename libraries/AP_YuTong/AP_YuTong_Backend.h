...
public:
	...
	AP_YuTong_Backend(YuTong::YuTong_State &_state, AP_YuTong_Params &_params);

    // we declare a virtual destructor so that YuTong drivers can
    // override with a custom destructor if need be
    virtual ~AP_YuTong_Backend(void) {}
    ...
    void get_state(YuTong::YuTong_State &state_arg);
    ...
    virtual bool handle_script_msg(const YuTong::YuTong_State &state_arg) { return false; }
#endif

#if HAL_MSP_YUTONG_ENABLED
    virtual void handle_msp(const MSP::msp_yutong_data_message_t &pkt) { return; }
#endif

    enum Rotation orientation() const { return (Rotation)params.orientation.get(); }
    
    float soc() const { return state.soc; }  
    float speed_kmh() const { return state.speed_kmh; }
    float motor_temperature() const { return state.motor_temperature; }
    uint8_t gear() const { return state.gear; }
    ...
    YuTong::Status status() const;
    YuTong::Type type() const { return (YuTong::Type)params.type.get(); }
    ...
    // return the actual type of the yutong, as opposed to the
    // parameter value which may be changed at runtime.
    YuTong::Type allocated_type() const { return _backend_type; }

protected:

    // update status based on distance measurement
    void update_status(YuTong::YuTong_State &state_arg) const;
    void update_status() { update_status(state); }

    // set status and update valid_count
    static void set_status(YuTong::YuTong_State &state_arg, YuTong::Status status);
    void set_status(YuTong::Status status) { set_status(state, status); }

    YuTong::YuTong_State &state;
    AP_YuTong_Params &params;

    // semaphore for access to shared frontend data
    HAL_Semaphore _sem;

    //Type Backend initialised with
    YuTong::Type _backend_type;
