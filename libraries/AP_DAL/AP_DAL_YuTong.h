...
public:
    ...
    class AP_DAL_YuTong_Backend *get_backend(uint8_t id) const;
...
private:
    ...
    AP_DAL_YuTong_Backend **_backend;
};


class AP_DAL_YuTong_Backend {
public:

    AP_DAL_YuTong_Backend(struct log_RRNI &RRNI);

    // YuTong-backend-like methods

    enum Rotation orientation() const {
        return (Rotation)_RRNI.orientation;
    }

    AP_DAL_YuTong::Status status() const {
        return (AP_DAL_YuTong::Status)_RRNI.status;
    }
    ...
    // DAL methods:
    void start_frame(AP_YuTong_Backend *backend);
